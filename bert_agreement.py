...
class BertEncoder(object):
    ...
    def encode(self, sentence: str, mask_index: int, layer: int, P: np.ndarray, P2: np.ndarray, alpha):
...
def collect_bert_states(bert, data: List[Tuple], layer=-1, P=None, P2=None, alpha=None):
    data_with_states = []
    ...
    for i, d in enumerate(data):
        ...
        predictions_dict, vec, top_words, orig2tok, bert_tokens = bert.encode(sent, mask_index=word_position,
                                                                              layer=layer, P=P, P2=P2, alpha = alpha)

        dict_data = d.copy()
        dict_data["vec"] = vec
        dict_data["top_preds"] = top_words[:3000]
